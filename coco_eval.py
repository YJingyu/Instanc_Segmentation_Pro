...
class CocoEvaluator(object):
    ...
    def update_inference(self, predictions):
        """
        ...
        for iou_type in self.iou_types:
            results[iou_type] = self.prepare(predictions, iou_type)
            # NOTE(rjbruin): Cut here? Accumulated `results` for all images,
            # store to file, then at eval time load the entire file, put all
            # image IDs in `self.img_ids` and run `update()` (once for all
            # images), `accumulate()` etc. (see `evaluate()``)
        return results

    def put_results(self, results_from_file):
        """
        ...
        """
        # Get all unique image IDs from the "prepared" format results
        img_ids = []
        for iou_type in self.iou_types:
            img_ids.extend([d['image_id'] for d in results_from_file[iou_type]])
        self.img_ids = list(np.unique(img_ids))

        for iou_type in self.iou_types:
            results = results_from_file[iou_type]
            # NOTE(rjbruin): Continue here with the results from file
...
def loadRes(self, resFile):
    ...
    # print(len(set(annsImgIds)), len(set(self.getImgIds())))
