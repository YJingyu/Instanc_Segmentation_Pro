...
class HybridTaskCascadeRoIHead(CascadeRoIHead):
    ...
    def __init__(self,
        ...
        # mask scoring
        self.mask_iou_head = torch.nn.ModuleList()
        if not isinstance(mask_iou_head, list):
            mask_iou_head = [mask_iou_head for _ in range(self.num_stages)]
        assert len(mask_iou_head) == self.num_stages
        for head in mask_iou_head:
            self.mask_iou_head.append(build_head(head))
    ...
    def _mask_forward_train(self,
        ...
        mask_feats = mask_roi_extractor(x[:mask_roi_extractor.num_inputs],
                                        pos_rois)                                        
        mask_iou_head = self.mask_iou_head[stage]
        ...
        mask_results = dict(loss_mask=loss_mask, mask_pred=mask_pred, mask_feats=mask_feats, mask_targets=mask_targets)

        # mask iou head forward and loss
        pos_mask_pred = mask_results['mask_pred'][range(mask_results['mask_pred'].size(0)), pos_labels]
        mask_iou_pred = mask_iou_head(mask_results['mask_feats'], pos_mask_pred)
        ...
        mask_iou_targets = mask_iou_head.get_targets(sampling_results, gt_masks,
                                                     pos_mask_pred, mask_results['mask_targets'], rcnn_train_cfg)
        ...
        if self.with_mask:
            ...
            if all(det_bbox.shape[0] == 0 for det_bbox in det_bboxes):
                ...
                for i in range(self.num_stages):
                    mask_head = self.mask_head[i]
                    # mask iou head-lengyu.yb
                    mask_iou_head = self.mask_iou_head[i]
                    if self.mask_info_flow:
                        mask_pred, last_feat = mask_head(mask_feats, last_feat)
                    else:
                        mask_pred = mask_head(mask_feats)
                    # mask iou head-lengyu.yb
                    mask_iou_pred = mask_iou_head(mask_feats,
                                                  mask_pred[range(concat_det_labels.size(0)), concat_det_labels])
                    # split batch mask prediction back to each image
                    mask_pred = mask_pred.split(num_bbox_per_img, 0)
                    # mask iou head-lengyu.yb
                    mask_iou_pred = mask_iou_pred.split(num_bbox_per_img, 0)
                    mask_iou_preds.append(mask_iou_pred)
                    aug_masks.append(
                ...
                for i in range(num_imgs):
                    if det_bboxes[i].shape[0] == 0:
                        ...
                        segm_result = self.mask_head[-1].get_seg_masks(
                            ...
                            rescale)
                        # mask iou head-lengyu.yb
                        mask_score = self.mask_iou_head[-1].get_mask_scores(
                            mask_iou_preds[-1][i], det_bboxes[i], det_labels[i])
                        mask_scores.append(mask_score)
                        segm_results.append(segm_result)
            # ms_segm_result['ensemble'] = segm_results
            if len(mask_scores) > 0:
                ms_segm_result['ensemble'] = list(zip(segm_results, mask_scores))
            else:
                ms_segm_result['ensemble'] = segm_results
        if self.with_mask:
            results = list(
        ...
        if self.with_mask:
            ...
            if det_bboxes.shape[0] == 0:
                ...
                for x, img_meta, semantic in zip(img_feats, img_metas,
                    ...
                    # mask iou head-lengyu.yb
                    mask_score = self.mask_iou_head[-1].get_mask_scores(
                        mask_iou_preds[-1], det_bboxes, det_labels)
                    mask_scores.append(mask_score)
            ...
            # mask iou head-lengyu.yb
            if len(mask_scores) > 0:
                # mask_score = [sum(list(map(lambda x: x[0], mask_scores)))/len(mask_scores)]
                mask_score = mask_scores[0]
                segm_result = (segm_result, mask_score)
